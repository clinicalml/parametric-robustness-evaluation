...
class DataLoader(object):
    ...
    def __init__(self,label_names,config):
        ...
        attributes = pd.read_csv(config.attr_file) #+-1
        #Store all labels for reference
        self.image_ids = attributes['image_id']
        self.all_attr= 0.5*(attributes.drop("image_id", axis=1)+1) # attributes is {0,1}
        ...
        self.image_dir=os.path.join(config.data_path,'img_align_celeba/img_align_celeba')
        self.filenames=[os.path.join(self.image_dir,j) for j in self.image_ids]
